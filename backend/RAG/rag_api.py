...
async def upload_reference(
    ...
    chunker: str = Form("recursive", enum=["recursive", "semantic"], description="Chunking strategy."),
    embedder: str = Form("gitee", enum=["openai", "gemini", "gitee"], description="Embedding model."),
):
    """
    ...
    return {"message": f"Reference document '{file.filename}' uploaded successfully for course '{course_id}'."}
...
async def get_feedback(
    ...
    course_id: str = Form(..., description="Course ID, e.g. 'MATH101'."),
    chunker: str = Form("recursive", enum=["recursive", "semantic"], description="Chunking strategy."),
    embedder: str = Form("gitee", enum=["openai", "gemini", "gitee"], description="Embedding model for document ingestion."),
    provider: str = Form("gitee", enum=["openai", "gemini", "gitee", "deepseek"], description="LLM provider for feedback generation."),
):
    """
    ...
    try:
        ...
        if not qvec:
            raise HTTPException(status_code=500, detail="Failed to ingest file and get query vector.")
